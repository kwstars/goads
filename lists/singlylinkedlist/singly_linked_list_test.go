...
func TestList_RemoveRange(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "remove range from a list with zero element",
			values:    []int{1},
			fromIndex: 1,
			toIndex:   1,
			wantErr:   nil,
...
func TestList_SubList(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			toIndex:   1,
			want:      nil,
			wantErr:   ErrFormIndexMustBeLessThanToIndex,
		},
		{
			name:      "get the whole list",
			values:    []int{1, 2, 3},
			fromIndex: 0,
			toIndex:   3,
			want:      []int{1, 2, 3},
