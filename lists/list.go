...
type List[T any] interface {
	...
	// PopAtIndex removes an element at a specific position in the list and returns it.
	PopAtIndex(index int) (T, error)

	// RemoveRange deletes a range of elements from the list, between two indices.
	// It returns an error if the operation is unsuccessful.
	RemoveRange(fromIndex int, toIndex int) error

	// RemoveIf deletes all elements in the list that satisfy a given predicate.
	RemoveIf(keep func(T) bool) bool
	...
	// Contains determines whether a list contains all elements in another list.
