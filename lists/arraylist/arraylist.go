...
// If the element is already at the front or does not exist in the list, no action is taken.
...
func (t *ArrayList[T]) BringElementToFront(element T) {
	...
	for i, currentElement := range t.elements {
		switch {
		case i == 0: //If at first element, set it to the target element
			t.elements[0] = element
			previousElement = currentElement //And set previousElement to the original first element
		case t.cmp(currentElement, element) == 0: //If found the target element
			t.elements[i] = previousElement //Set this position to previousElement
			return                          //And return
		default: //Otherwise
			t.elements[i] = previousElement  //Set this position to previousElement
			previousElement = currentElement //And continue traversing by setting previousElement to the current element
		}
	}
	t.elements = append(t.elements, previousElement) //If reached the end without finding the target element,append it
}
